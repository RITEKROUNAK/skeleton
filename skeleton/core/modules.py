...
class CustomModuleLoader:
    ...
    def __init__(self, folder='skeleton/modules', blacklist=[], options=None, logger=logging.INFO, database=None, scope=None):
    ...
    def load(self, f):
        base = f.replace('.py', '')
        try:
            command_module = __import__("skeleton.modules.%s" % base, fromlist=["modules"])
