...
```
git clone https://github.com/RITEKROUNAK/skeleton.git
cd skeleton
...
```
usage: skeleton: args [-h] [-u URL] [--urls URLS]
...
skeleton -u "http://example.com/" -c -s

Safe scan
skeleton -u "http://example.com/" -c -s --options "passive,discovery" --adv

Full scan (with unsafe scripts)
skeleton -u "http://example.com/" -a --options all --max-urls 1000

Scan a single URL
skeleton -u "http://example.com/vuln.php?id=1" -s

Scan webapps and CMS systems
skeleton -u "http://example.com/blog/" --webapp --cms

Pwn a web server
skeleton -u "http://example.com/" --msf
```

skeleton is built for authorized use only!
